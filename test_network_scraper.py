#!/usr/bin/env python3
"""
Test script for the new Network Interception-based Vinted scraper
"""
import os
import sys
import logging
...
# Set up environment for network interception mode
os.environ['VINTED_SCRAPER_MODE'] = 'network'  # Force Network Interception mode
...
def test_basic_scraper_initialization():
    """Test that the network interception scraper can be initialized"""
    logger.info("🧪 Testing network interception scraper initialization...")
    
    try:
        scraper = VintedScraper("https://www.vinted.be")
        logger.info("✅ Network interception scraper initialized successfully")
        return scraper
...
def test_simple_search(scraper):
    """Test a simple search with network interception"""
    logger.info("🔍 Testing simple search with network interception...")
    ...
    try:
        ...
        if isinstance(results, dict) and 'items' in results:
            ...
            logger.info(f"✅ Network interception search successful - found {item_count} items")
        ...
        logger.error(f"❌ Network interception search test failed: {e}")
        return False


def test_search_with_parameters(scraper):
    """Test search with specific parameters using network interception"""
    logger.info("🔍 Testing parameterized search with network interception...")
    ...
    try:
        ...
        if isinstance(results, dict) and 'items' in results:
            item_count = len(results['items'])
            logger.info(f"✅ Parameterized network interception search successful - found {item_count} items")
        ...
        logger.error(f"❌ Parameterized network interception search test failed: {e}")
        return False


def test_item_details(scraper, item_id="1234567890"):
    """Test fetching item details with network interception"""
    logger.info(f"📦 Testing item details with network interception for ID: {item_id}...")
...
def run_tests():
    """Run all network interception tests"""
    logger.info("🚀 Starting Network Interception scraper tests...")
    ...
    # Print results
    logger.info("\n" + "="*50)
    logger.info("🧪 NETWORK INTERCEPTION TEST RESULTS:")
    logger.info("="*50)
    ...
    if passed == total:
        logger.info("🎉 All tests passed! Network interception scraper is working correctly.")
        return True
    else:
